		...
		for (int i = 0; i < widgets.size; ++i)
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != mt || tex != tx || sdr != sd)
				{
					...
					{
                        // 好像可以放到后面，放到这里的目的是：
                        // 如果多个widget共用一个mat，uv，shader，只需在最后一次才Add到dcs
                        // 也是因为先做mat ！= mt判断
						drawCalls.Add(dc);
						dc.UpdateGeometry();
						dc = null;  // Create后，清空dc
					}
				...
				if (mat != null || sdr != null || tex != null)
				{
					...
					{
                        // 共用前面的dc，但更新depthStart/End
                        // 与前一个mat，uvs相等，则复用前一个dc  
                        // 多个widget，他们depth是在一个区间的（start,end)
						int rd = w.depth;
		...
		for (int i = 0; i < drawCalls.size; ++i)
		{
			...
			if (dcStart <= depth && dcEnd >= depth)
			{
				if (dc.baseMaterial == mat && dc.mainTexture == tex)
				{
					if (w.isVisible)
					{
						w.drawCall = dc;    // widget指定一个dc（相同的mat和tex）
