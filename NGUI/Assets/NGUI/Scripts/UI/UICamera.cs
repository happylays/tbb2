...
public class UICamera : MonoBehaviour
{
	...
	public void ProcessMouse ()
	{
		...
		if (isPressed || posChanged || mNextRaycast < RealTime.time)
		{
			...
			if (!Raycast(Input.mousePosition, out lastHit)) hoveredObject = fallThrough;    // 1.获取鼠标上的Object
			if (hoveredObject == null) hoveredObject = genericEventHandler;
			for (int i = 0; i < 3; ++i) mMouse[i].current = hoveredObject;  // 2. mouse.current = object
		}
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			currentTouch = mMouse[i];   // 3.获取mouse点击的Object
			currentTouchID = -1 - i;
			...
			ProcessTouch(pressed, unpressed);   // 4.处理Touch
	...
	public void ProcessTouch (bool pressed, bool unpressed)
	{
		...
		else if (currentTouch.pressed != null && (currentTouch.delta.sqrMagnitude != 0f || currentTouch.current != currentTouch.last))
		{
			...
			if (!currentTouch.dragStarted && currentTouch.last != currentTouch.current) // 拖拽处理
			{
			...
			if (currentTouch.dragStarted)   // 发送拖拽事件
			{
				if (mTooltip != null) ShowTooltip(false);
				...
				{
					Notify(currentTouch.dragged, "OnDragStart", null);
					Notify(currentTouch.current, "OnDragOver", currentTouch.dragged);   // disable pressed color
				}
				else if (currentTouch.last != currentTouch.current)
				{
					Notify(currentTouch.last, "OnDragOut", currentTouch.dragged);   // disable normal color
