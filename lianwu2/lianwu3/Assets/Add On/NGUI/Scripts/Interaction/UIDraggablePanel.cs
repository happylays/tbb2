...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	/// <summary>
	/// fufeng modify: how the dragging will be restricted to be within the parent panel's bounds.
	/// </summary>

	public RestrictMode restrictPanelMode = RestrictMode.None;
	...
	public OnDragFinished onDragFinished = null;
	...
	Vector4 mOrgClipRange = Vector4.zero;
	Vector3 mOrgPanelPos = Vector3.zero;
	...
	/// <summary>
	/// fufeng add: flag to adjust position when update.
	/// </summary>
	bool mNeedAdjust = false;
	Vector3 mAdjsutOffset = Vector3.zero;
	...
	/// <summary>
	/// fufeng add: check whether panel is being pressed
	/// </summary>
	...
	void Awake ()
	{
        ...
        mOrgClipRange = mPanel.clipRange;
		mOrgPanelPos = mPanel.transform.localPosition;
	}
	...
	public bool RestrictWithinBounds (bool instant)
	{
		...
		if ( restrictPanelMode != RestrictMode.None )
		{
			...
			{
				if ( minBounds.x > minArea.x ) constraint.x = minArea.x - minBounds.x;
				if ( minBounds.y > minArea.y ) constraint.y = minArea.y - minBounds.y;
			}
			else if ( restrictPanelMode == RestrictMode.PositiveEdge )
			{
				if ( maxBounds.x < maxArea.x ) constraint.x = maxArea.x - maxBounds.x;
				if ( maxBounds.y < maxArea.y ) constraint.y = maxArea.y - maxBounds.y;
			}
		}
		
		// fufeng modify: restrict only in scale
		if ( Mathf.Approximately( scale.x, 0f ) ) constraint.x = 0f;
		if ( Mathf.Approximately( scale.y, 0f ) ) constraint.y = 0f;
		if ( Mathf.Approximately( scale.z, 0f ) ) constraint.z = 0f;
	...
	public void SetDragAmount (float x, float y, bool updateScrollbars, Bounds b)
	{
		DisableSpring();
	...
	public void ResetPosition()
	{
		...
		Bounds b = bounds;
		if (b.min.x == b.max.x || b.min.y == b.max.x)
		{
			mPanel.clipRange = mOrgClipRange;
			Vector3 pos = mTrans.localPosition;
			pos.x = mOrgPanelPos.x;
			pos.y = mOrgPanelPos.y;
			mTrans.localPosition = pos;
		...
		else
		{
			...
			SetDragAmount(relativePositionOnReset.x, relativePositionOnReset.y, false, b);

			// Next move the clipping area back and update the scroll bars
			SetDragAmount(relativePositionOnReset.x, relativePositionOnReset.y, true, bounds);
		}
	}
	...
	void OnHorizontalBar (UIScrollBar sb)
	{
		if (!mIgnoreCallbacks)
		{
			...
			SetDragAmount(x, y, false, bounds);
		}
	}
	...
	public void MoveRelative (Vector3 relative, bool updateBounds)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				else
				{
					...
					if (constraint.magnitude > 0.001f)
					{
						MoveAbsolute(offset * 0.5f, false);
						mMomentum *= 0.5f;
					}
					else
					{
	...
	void LateUpdate ()
	{
		...
		if (mShouldMove && !mPressed)
		{
			...
			else
			{
                ...
                //debug if ( mNeedAdjust && m_GridAdjust != null && mPanel != null )
                if (mNeedAdjust && mPanel != null)
				{
					//debug mAdjsutOffset = m_GridAdjust.GetDockOffset();
					mAdjsutOffset.Scale( scale );

					mNeedAdjust = false;
				}
						
				if ( mAdjsutOffset.magnitude > 0.000001f )
				{
					// Move the panel
					Vector3 offSet = NGUIMath.SpringLerp( Vector3.zero, mAdjsutOffset, 20f, delta );
					MoveAbsolute( offSet, false );

					mAdjsutOffset -= offSet;
	...
	public void JumpToDestPosition(Vector3 destPos, Vector3 dockPos)
	{
		Vector3 srcPos = transform.InverseTransformPoint(destPos);
		Vector3 desPos = transform.InverseTransformPoint(dockPos);
		Vector3 offset = new Vector3(0, desPos.y - srcPos.y, 0);

		DisableSpring();
		MoveRelative(offset, true);
		RestrictWithinBounds(true);
	}
}
