...
public class UIInput : MonoBehaviour
{
	...
	float mPosition = 0f;
	int mChineseByteCount = 2;	//one char occupy two byte counts
	...
	/// <summary>
	/// Get current input text length, english char is 1, chinese char is 2.
	/// </summary>
	public int curCharCount
	{
		get
		{
			...
			List<byte[]> wordList = CommonFunc.GetString2Bytes(mText, CommonFunc.GetCharsetEncoding());
			foreach (byte[] arr in wordList)
			{
				if (arr.Length > mChineseByteCount)
				{
					wordLength += mChineseByteCount;
				}
				else
				{
	...
	void UpdateLabel ()
	{
		...
		{
			int wordLength = 0;
			List<byte> wordByteArr = new List<byte>();
			...
			foreach (byte[] arr in wordList)
			{
				...
				{
					mText = Encoding.UTF8.GetString(wordByteArr.ToArray());
					break;
				}

				wordByteArr.AddRange(arr);
			}
		}
	...
	public void ResetLabelPosX(float x)
	{
		mPosition = x;
	}
}
