...
Shader "DDL/Scene/MADFINGER/Environment/Scroll 2 Layers Multiplicative No Lightmap Sine" {
Properties {
...
SubShader {
	...
	float _SineFreqX2;
	float _SineFreqY2;
	float _GlobalColorFactor;
	
	struct v2f {
		float4 pos : SV_POSITION;
		float4 uv : TEXCOORD0;
		fixed4 color : TEXCOORD1;
		fixed  Factor : TEXCOORD2;
	};
	...
	Pass {
		...
		fixed4 frag (v2f i) : COLOR
		{
			...
			return o * _DDL_Global_Add_Color;
		}
		ENDCG 
	}	
}
